...
ADDRESS_BCRT1_UNSPENDABLE = 'gfcnrt1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqk0jtm6'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(gfcnrt1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqk0jtm6)#70vh8kqv'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'gfcnrt1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsqhsn2cu'
...
def create_deterministic_address_bcrt1_p2tr_op_true(explicit_internal_key=None):
    ...
    if explicit_internal_key is None:
        assert_equal(address, 'gfcnrt1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqs7lnjf4')
...
def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    version = 36 if main else 95
    return byte_to_base58(hash, version)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    version = 16 if main else 197
    return byte_to_base58(hash, version)
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address("gfcn" if main else "gfcnrt", version, program)
...
def bech32_to_bytes(address):
    ...
    if hrp not in ['gfcn', 'tgfcn', 'gfcnrt']:
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    if version in (36, 95):  # pubkey hash (main, or testnet/regtest)
        return keyhash_to_p2pkh_script(payload)
    elif version in (16, 197):  # script hash (main, or testnet/regtest)
