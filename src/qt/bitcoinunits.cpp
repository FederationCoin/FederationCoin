...
QList<BitcoinUnit> BitcoinUnits::availableUnits()
{
    ...
    unitlist.append(Unit::COIN);
    unitlist.append(Unit::TOKEN);
    return unitlist;
}
...
{
    switch (unit) {
    case Unit::COIN: return QString("COIN");
    case Unit::TOKEN: return QString("token");
...
{
    switch (unit) {
    case Unit::COIN: return QString("100 million tokens (1 COIN)");
    case Unit::TOKEN: return QString("one token (atomic unit)");
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
{
    switch (unit) {
    case Unit::COIN: return COIN;
    case Unit::TOKEN: return 1;
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
{
    switch (unit) {
    case Unit::COIN: return 8;
    case Unit::TOKEN: return 0;
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}

qint64 BitcoinUnits::singlestep(Unit)
{
    return 100000;
}
...
QString BitcoinUnits::format(Unit unit, const CAmount& nIn, bool fPlus, SeparatorStyle separators, bool justify)
{
    ...
    QString quotient_str = QString::number(quotient, 10);
    if (justify) {
    ...
    if (num_decimals > 0) {
        ...
        remainder_str = QString::number(remainder, 10).rightJustified(num_decimals, '0');
    }
...
bool BitcoinUnits::parse(Unit unit, const QString& value, CAmount* val_out)
{
    ...
    CAmount retvalue(str.toLongLong(&ok, 10));
    if(val_out)
    {
        *val_out = retvalue;
    }
...
qint8 BitcoinUnits::ToSetting(BitcoinUnit unit)
{
    switch (unit) {
    case BitcoinUnit::COIN: return qint8{0};
    case BitcoinUnit::TOKEN: return qint8{1};
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
{
    switch (num) {
    case 0: return BitcoinUnit::COIN;
    case 1: return BitcoinUnit::TOKEN;
    }
    return BitcoinUnit::COIN;
}
} // namespace

BitcoinUnit BitcoinUnits::FromSetting(const QString& s, BitcoinUnit def)
{
    if (s == "0" || s == "COIN") return BitcoinUnit::COIN;
    if (s == "1" || s == "SEC" || s == "TOKEN") return BitcoinUnit::TOKEN;
    return def;
}

QDataStream& operator<<(QDataStream& out, const BitcoinUnit& unit)
{
    return out << BitcoinUnits::ToSetting(unit);
}
