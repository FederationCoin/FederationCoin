...
class AcceptBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for i in range(2):
            blocks_h2.append(create_block(tips[i], create_coinbase(2), block_time, height=2))
            blocks_h2[i].solve()
            block_time += 1
        ...
        block_h1f = create_block(int("0x" + self.nodes[0].getblockhash(0), 0), create_coinbase(1), block_time, height=1)
        block_time += 1
        block_h1f.solve()
        ...
        block_h2f = create_block(block_h1f.sha256, create_coinbase(2), block_time, height=2)
        block_time += 1
        block_h2f.solve()
        ...
        block_h3 = create_block(block_h2f.sha256, create_coinbase(3), block_h2f.nTime+1, height=3)
        block_h3.solve()
        ...
        all_blocks = []
        for i in range(288):
            next_block = create_block(tip.sha256, create_coinbase(i + 4), tip.nTime+1, height=i + 4)
            next_block.solve()
        ...
        block_289f = create_block(all_blocks[284].sha256, create_coinbase(289), all_blocks[284].nTime+1, height=289)
        block_289f.solve()
        block_290f = create_block(block_289f.sha256, create_coinbase(290), block_289f.nTime+1, height=290)
        block_290f.solve()
        # block_291 spends a coinbase below maturity!
        tx_to_add = create_tx_with_script(block_290f.vtx[0], 0, script_sig=b"42", amount=1)
        block_291 = create_block(block_290f.sha256, create_coinbase(291), block_290f.nTime+1, txlist=[tx_to_add], height=291)
        block_291.solve()
        block_292 = create_block(block_291.sha256, create_coinbase(292), block_291.nTime+1, height=292)
        block_292.solve()
        ...
        block_293 = create_block(block_292.sha256, create_coinbase(293), block_292.nTime+1, height=293)
        block_293.solve()
        headers_message = msg_headers()
